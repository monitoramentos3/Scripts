...
def pedir_hora_referencia():
    root = Tk()
    root.withdraw()  # Esconder janela principal
    while True:
        hora_str = simpledialog.askstring(
            "Hora de Referência", 
            "Digite a hora de referência (0-23) ou deixe em branco para usar a hora atual:"
        )
        if hora_str is None:
            # Cancelou: usar hora atual
            return None
        ...
        if hora_str.isdigit() and 0 <= int(hora_str) <= 23:
            ...
            messagebox.showerror("Erro", "Por favor, digite um número válido entre 0 e 23.")

def main():
    # Pede hora de referência via popup
    hora_ref = pedir_hora_referencia()
    if hora_ref is None:
    ...
    for aba in abas:
        try:
            ...
            elif data_descarga.date() != agora.date():
                status = "Não descarregou"
            else:
                if data_descarga.hour >= hora_ref:
                    status = "Descarregou"
                else:
                    status = "Não descarregou"
